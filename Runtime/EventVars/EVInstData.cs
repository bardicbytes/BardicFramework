...
namespace BardicBytes.BardicFramework.EventVars
{
    ...
    public class EVInstData
    {
        ...
        public EVInstData(EventVar src)
        {
            ActorInstance = null;
            SetSrc(src);
            editorName = src.name;
        ...
        private void SetSrc(EventVar evSrc)
        {
            ...
            if (selector == typeof(int).FullName)
            {
                ...
                //Debug.Log(evSrc.GetType().Name +" SetSrc int " +IntValue);
                return;
        ...
        public virtual bool PropField(Rect position, UnityEditor.SerializedProperty evifProp)
        {
            ...
            else if (selector == typeof(float).FullName && DrawPF("BoolValue"))
            {
                var p = FindPropRel("BoolValue");
                if (p != null) BoolValue = p.boolValue;
            }
            else if (selector == typeof(bool).FullName && DrawPF("FloatValue"))
            {
